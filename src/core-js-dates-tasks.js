...
function getNextFriday(date) {
  ...
  let offset = (FRIDAY - currentDay + 7) % 7;
  if (offset === 0) {
    offset = 7;
  }
...
function getCountWeekendsInMonth(month, year) {
  ...
  const weekendsFromFullWeeks = fullWeeks * 2;
  const remainingDays = daysInMonth % 7;
  const leftoverWeekendDays = Array.from({ length: remainingDays })
    .map((_, index) => (firstDay + index) % 7)
    .filter((weekday) => weekday === 0 || weekday === 6).length;

  return weekendsFromFullWeeks + leftoverWeekendDays;
}
...
function getNextFridayThe13th(date) {
  function find(year, month) {
    ...
    return find(nextYear, nextMonth);
  }

  return find(date.getFullYear(), date.getMonth());
}
